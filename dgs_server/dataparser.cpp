...
bool DataParser::checkJSON(QByteArray data)
{
    ...
    QJsonDocument document12(jsonObj);
    emit sendToLogger(document12.toJson(), m_Socket->socketDescriptor());

    emit sendToHandler(jsonObj, QUuid(msgId));
    return true;
}

bool DataParser::isProperPayload(QJsonObject payload)
{
    if(payload.value(QString("ObjType")).toString() == QString("ArrayPayload")||
       payload.value(QString("ObjType")).toString() == QString("MsgResponse")){
        return false;
    }
